...
// Ensure the SimpleMenuItem table is seeded.
// The table is created by Prisma during build (prisma db push),
// so we only need to seed default items if the table is empty.
export async function ensureSimpleMenuItemTable(): Promise<boolean> {
  ...
  setupPromise = (async () => {
    ...
    try {
      // Table already exists via Prisma schema (prisma db push during build).
      // Just check if it needs seeding.
      ...
      console.error('[DB Setup] Failed to access SimpleMenuItem table:', error);
      return false;
    }
  })();
